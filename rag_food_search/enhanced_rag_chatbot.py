...
model = ChatOllama(model="phi", temperature=0.1)
...
def main():
    ...
    try:
        ...
        print(
            f"LLM Test Response: {getattr(test_response, 'content', str(test_response))}"
        )
        if test_response and getattr(test_response, "content", None):
...
Response:'''
        ...
        response_text = getattr(generated_response, "content", "").strip()
...
def enhanced_rag_food_chatbot(collection):
    ...
    print("   Powered by Ollama Phi Model & ChromaDB")
    print("=" * 70)
...
def handle_enhanced_rag_query(collection, query: str, conversation_history: list[str]):
    ...
    for i, result in enumerate(search_results[:3], 1):
        ...
        print(
            f"   📍 {result['cuisine_type']} | 🔥 {result['food_calories_per_serving']} cal | 📈 {result['similarity_score'] * 100:.1f}% match"
        )
        if i < 3:
            print()
...
def handle_enhanced_comparison_mode(collection):
    ...
    print("=" * 60)
    print(
        f"{'Query 1: ' + query1[:20] + '...' if len(query1) > 20 else 'Query 1: ' + query1:<30} | {'Query 2: ' + query2[:20] + '...' if len(query2) > 20 else 'Query 2: ' + query2}"
    )
    print("-" * 60)
...
Comparison:'''
        ...
        return print(f"❌ LLM Comparison Error: {e}")
